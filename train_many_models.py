...
def main():
    ...
    if args.training_type == 'linear-classifier':        
        ...
        base_path = 'sim_distill_cifar_from_resnet50'
        student_path_list = os.listdir(base_path)
        student_path_list = [os.path.abspath(os.path.join(base_path,path)) for path in student_path_list]    
        
        if args.rename:
            rename_models(root_path = base_path, path_list = student_path_list)
...
def add_linear_classifier_to_many(student_type : str,path_list : list,epoch_numbers :list, dataset:str,devices : list):
    ...
    for i,obj in enumerate(zip(epoch_numbers,path_list)):
        epoch,path = obj
        ...
        bash_string = f"python3 run_training.py --mode linear_classifier --dataset {dataset} --device {device} \
                ...
                --lr 0.1 --optimizer sgd --batch-size 128 --epochs {epochs} --early-stop 25"  
...
def distill_many_students(base_teacher_path : str,teacher_type : str,student_type : str,epoch_numbers :list ,teacher_models : list,sim_distillation : bool,dataset : str,devices : list):
    ...
    for i,obj in enumerate(zip(epoch_numbers,teacher_models)):
